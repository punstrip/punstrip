...
##TODO: test label=True, need node attribute "label"="{{label}}"

class GK_WL():

    def _build_adjacency_matrix(self, G):
        nodes = list(G.nodes())
        n = len(list(G.nodes()))
        m = np.zeros((n, n))
        #print(m)
        for a, bnodes in G.adjacency():
            for b in bnodes:
                m[nodes.index(a),nodes.index(b)] += 1
        return m
    ...
    def compare_list(self, graph_list, h=1, node_label=True):
        ...
        #print("graph_list[0]".format(graph_list[0].nodes()))
        #print(graph_list[0])
        lists = [0] * n
        k = [0] * (h + 1)
        n_nodes = 0
        ...
        for i in range(n):
            #print("Graph list {} : {}".format(i,  graph_list[i] ))
            #lists[i] = list( graph_list[i].adjacency() )
            lists[i] = self._build_adjacency_matrix( graph_list[i] )
        ...
        phi = np.zeros((n_max, n), dtype=np.float64)
        ...
        if node_label is True:
            ...
            for i in range(n):
                #degree is number of in + out which can be bigger than number of nodes.
                #labels[i] = np.array(list(map(lambda x: x[1], graph_list[i].degree())) )
                labels[i] = np.array(list(map(lambda x: x[1], graph_list[i].out_degree())) )
                for j in range(len(labels[i])):
                    ### nodes degree can be between 0 - max size, i.e. max size + 1 entries
                    assert( labels[i][j] >= 0 )
                    #assert( labels[i][j] < n_max )
                    if labels[i][j] >= n_max:
                        phi[n_max - 1, i] += 1
                    else:
                        phi[labels[i][j], i] += 1
        ...
        while it < h:
            ...
            phi = np.zeros((n_nodes, n), dtype=np.float64)
            for i in range(n):
