...
class Symbol(BasicBlockSequence):
    ...
    def to_vec(self, E, KNOWN_FUNCS=[]):
        ...
        callees_vec     = reduce(lambda x,y: x + y, map(lambda x: E.to_vec('ml_name_vector', nlp.canonical_set(x)), mod_callees),
            E.to_vec('ml_name_vector', []))
        callers_vec     = reduce(lambda x,y: x + y, map(lambda x: E.to_vec('ml_name_vector', nlp.canonical_set(x)), mod_callers),
            E.to_vec('ml_name_vector', []))
        closure_vec     = reduce(lambda x,y: x + y, map(lambda x: E.to_vec('ml_name_vector', nlp.canonical_set(x)), mod_closure),
            E.to_vec('ml_name_vector', []))
        ...
        t_flow_vec = E.to_vec('ml_name_vector', [])
        for flow in self.tainted_flows:
            ...
            try:
                ...
                t_vec = E.to_vec('ml_name_vector', nlp.canonical_set(func))
        ...
        consts_vec  = E.to_vec('constants_vector', id_consts)

        common_hashes       = [ self.opcode_hash ] if self.opcode_hash in E.hashes else []
        opcode_hashes_vec   = E.to_vec('hashes_vector', common_hashes)

        #return np.hstack( [vec, func_args_vec, operations_vec, jumpkinds_vec, callees_vec, callers_vec, closure_vec, consts_vec, opcode_hashes_vec , cfg_vec] ).reshape(1, -1)
        return np.hstack( [vec, func_args_vec, operations_vec, jumpkinds_vec, callees_vec, callers_vec, closure_vec, opcode_hashes_vec, num_flow_vec, t_flow_vec, cfg_vec, consts_vec] ).reshape(1, -1)
        #return np.hstack( [vec, func_args_vec, operations_vec, jumpkinds_vec, callees_vec, callers_vec, closure_vec, opcode_hashes_vec, num_flow_vec, t_flow_vec, cfg_vec ] ).reshape(1, -1)


    def to_context_vec(self, E, binary):
        """
