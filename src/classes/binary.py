...
class Binary:
        ...
        def __init__(self, config, name="", size=-1, data=b'', symbols=[], objects=[],
            ...
            if not os.path.isfile(path):
                raise Exception("Error, path argument to binary ({}) is not an EXEcutable file!".format(path))
        ...
        def r2_read_bytes(self, vaddr, size_in_bytes):
            """
            bits = self.r2_hdlr.cmd('pB {} @ {}'.format(size_in_bytes, vaddr))
            if len(bits) == 1 + (size_in_bytes*8):
                bits = bits[:-1]

            assert(len(bits) == size_in_bytes*8)
            return bytes(classes.utils.str_to_byte_ints(bits))
            """
            if size_in_bytes <= 0:
                return bytes()
            return bytes(json.loads(self.r2_hdlr.cmd('pxj {} @ {}'.format(size_in_bytes, vaddr))))
            ...
            for f in ri:
                if f['vaddr'] == 0:
                    ...
                    if f['name'] in relocs:
                        ...
                        continue
                    raise RuntimeError("Import has vaddr of 0x0000 and we cannot find it in the relocation table")
                self.dyn_imports.append(f)
...
if __name__ == '__main__':
    ...
    b       = Binary(config, path=sys.argv[1])
    b.analyse()
