...
class PostgresDB(Database):
    ...
    def binary_symbols(self, binary):
        symbols = []
        curr = self.conn.cursor()
        curr.execute("SELECT binary_id, public.binary.path, public.binary_functions.name, arguments, heap_arguments, tls_arguments, local_stack_bytes, num_args, return, closure, callees, callers, public.binary_functions.sha256, opcode_hash, asm_hash, public.binary_functions.size, binding, vex, cfg, tainted_flows, public.binary.dynamic_imports FROM public.binary_functions LEFT JOIN public.binary ON binary_id=public.binary.id WHERE public.binary.path = %s", (binary,))
        known_functions = set()
        for r in curr.fetchall():
            ...
            s = classes.symbol.Symbol(self.config, name=name, closure=closure, local_stack_bytes=local_stack_bytes,
    ...
    def resolve_dynamic_function(self, curr, lib_ids, regex_pattern):
        ...
        """
        curr.execute("""SELECT real_name, name, arguments, heap_arguments, tls_arguments, num_args, local_stack_bytes, return FROM library_prototypes WHERE
            library = ANY(%s) AND name LIKE %s""", (lib_ids, regex_pattern,))
        res = curr.fetchall()
        for f in res:
            d = {
                    'name' : f[1], 'real_name': f[0],
    ...
    def all_callees(self):
        callees = set([])
        curr = self.conn.cursor()
        curr.execute("SELECT DISTINCT(a) FROM (SELECT JSONB_ARRAY_ELEMENTS(callees) as a FROM public.binary_functions) AS foo")
    ...
    def all_callers(self):
        callers = set([])
        curr = self.conn.cursor()
        curr.execute("SELECT DISTINCT(a) FROM (SELECT JSONB_ARRAY_ELEMENTS(callers) as a FROM public.binary_functions) AS foo")
        for row in curr.fetchall():
            callers.add(row[0])
        return callers
    ...
    def constants_freq(self, max_freq=5, min_freq=3):
        curr = self.conn.cursor()
        curr.execute("SELECT t.count, t.const FROM (SELECT COUNT(*) as count, jsonb_array_elements(vex->'constants') as const FROM public.binary_functions GROUP BY const) t WHERE t.count < %s AND t.count > %s;", (max_freq, min_freq))
        freq = {}
