...
class BasicBlockSequence():
    ...
    def compute_pmf(self, proj):
        #weightings = [ 0.3, 5.0, 2.5, 0.15, 0.3, 1.0, 0.3, 1.0 ]
        projection = { 'vex' : 0.0, 'hash' : 0.0, 'opcode_hash': 0.0, 'vex.ntemp_vars' : 0.0, 'vex.ninstructions': 0.0, 'callees': 0.0, 'callers': 0.0, 'size': 0.0, 'cfg' : 0.0 }
        #weightings = { 'vex' : 1.0, 'hash' : 20.0, 'opcode_hash': 10.0, 'vex.ntemp_vars' : 0.01, 'vex.ninstructions': 0.05, 'callees': 1.5, 'callers': 0.1, 'size': 0.3, 'cfg': 3.0 }
        #weightings = {'size': 0.3915302522297891, 'hash': 0.6515103530084538, 'opcode_hash': 0.6712701352303624, 'vex': 5.4203195348508783, 'cfg': 0.5829651832309078, 'vex.ntemp_vars' : 0.1, 'vex.ninstructions' : 0.1, 'callees' : 0.2} 
        weightings = {'size': 0.03915302522297891, 'hash': 0.6515103530084538, 'opcode_hash': 0.6712701352303624, 'vex': 5.4203195348508783, 'cfg': 0.0829651832309078, 'vex.ntemp_vars' : 0.5, 'vex.ninstructions' : 0.8, 'callees' : 0.02} 
        ...
        if projection['size'] == 1:
            sel_hyper_r = P.exp_neg_x(10, 10, self.size)
            r = base_ratio * (1.0+ sel_hyper_r)
            pmfs['size'] = P._compute_gaussian_avg_pmf(db, "size", self.size, sigma_ratio=r, selection_ratio=r )
            pass

        if projection['vex.ntemp_vars'] == 1:
            sel_hyper_r = P.exp_neg_x(10, 10, self.vex['ntemp_vars'])
            r = base_ratio * (1.0+ sel_hyper_r)
            pmfs['vex.ntemp_vars'] = P._compute_gaussian_avg_pmf(db, "vex_ntemp_vars", self.vex['ntemp_vars'], selection_ratio=r, sigma_ratio=r )

        if projection['vex.ninstructions'] == 1:
            sel_hyper_r = P.exp_neg_x(10, 10, self.vex['ninstructions'])
            r = base_ratio * (1.0+ sel_hyper_r)
            pmfs['vex.ninstructions'] = P._compute_gaussian_avg_pmf(db, "vex_ninstructions", self.vex['ninstructions'], selection_ratio=r, sigma_ratio=r )

        if projection['callers'] == 1:
            #sigma_ratio should be high when self.callers len is low
            sel_hyper_r = P.exp_neg_x(10, 10, len(self.callers))
            r = base_ratio * (1.0+ sel_hyper_r)
            pmfs['callers'] = P._compute_gaussian_avg_pmf(db, "callers", len(self.callers), sigma_ratio=r, selection_ratio=r )

        if projection['callees'] == 1:
            sel_hyper_r = P.exp_neg_x(10, 10, len(self.callees))
            r = base_ratio * (1.0+ sel_hyper_r)
            pmfs['callees'] = P._compute_gaussian_avg_pmf(db, "callees", len(self.callees), sigma_ratio=r, selection_ratio=r )

        if projection['hash'] == 1:
            pmfs['hash'] = P.load_pmf(db, "hash", self.hash)

        if projection['opcode_hash'] == 1:
            pmfs['opcode_hash'] = P.load_pmf(db, "opcode_hash", self.opcode_hash)

        if projection['vex'] == 1:
            vex = { 
                'operations'    : self.vex['operations'], 
                'expressions'   : self.vex['expressions'], 
                'statements'    : self.vex['statements'], 
            }
            pmfs['vex'] = P.pmf_from_vex( db, vex, 20)

        if projection['cfg'] == 1:
            pmfs['cfg'] = P.pmf_from_cfg(db, self._id, 10)


        for k, v in pmfs.items():
            assert(v.sum() >= 0.0)
            if v.sum() < 0.001:
                logger.warning("PMF for {} tells us no information...".format(k) )

        #for k in pmfs.keys():
        #    pmfs[k] = P.add_uniform_error_and_normalise( pmfs[k] )

        base_pmf = np.zeros( (P.dim, 1), dtype=np.float64)
        for k, v in pmfs.items():
            ndp = P.normalise_numpy_density(v)
            mod_pmf = np.multiply( weightings[k], ndp )
            base_pmf = np.add( base_pmf, mod_pmf )

        return P.add_uniform_error_and_normalise_density( base_pmf )
...
class ConstantPropagation(BasicBlockSequence):
    ...
    def propagate(self, see):
        ...
        while len(worklist) > 0:
            ...
            """
                BUG: Remove all instances of node from teh worklist otherwise infinite cycles from 2 loops can occour
            """
...
class TaintPropagation(BasicBlockSequence):
    ...
    def latest_reg_name(self, reg, _bb_ind, resolved):
        ...
        while highest == 0:
            ...
            if bb_ind < 0:
                print("Error, invalid bb_ind for latest_reg_name")
                IPython.embed()
